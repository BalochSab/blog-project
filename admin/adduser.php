...
if (isset($_POST["adduser"])) {
    ...
    if (empty($_POST["user_role"])) {
        $ex['eurole'] = "Role is not selected";
    }else{
    ...
    if (!array_filter($ex)){
     ...
     require("connect.php");
     $check_user = "SELECT user_name FROM user WHERE user_name = '".$uname."'";
        $ucheck = mysqli_query($con, $check_user);
        $udata = mysqli_fetch_array($ucheck, MYSQLI_NUM);
        if ($udata[0]) {
            $ex['err'] =  "User Name Already in Exists<br/>";
        }
        else{
            $add_user = "INSERT INTO user value('NULL','".$fname."','".$lname."','".$uname."','".$upass."','".$urole."')";
        ...
        <div class="col-md-6">    
                ...
                <div class="card-body">
                    ...
                    <div class="form-group ">
                        ...
                        <input type="text"  name="first_name" value="<?php echo (isset($user_update_id)) ?  $updata['first_name'] : $fname; ?>" class="form-control" id="InputFirst" placeholder="Enter First Name">
                    <div class="text-danger"><?php echo $ex['efname'];?></div>
                    </div>
                    <div class="form-group ">
                        <label for="InputLast">Last Name</label>
                        <input type="text"  name="last_name" value="<?php  echo (isset($user_update_id)) ?  $updata['last_name'] : $lname; ?>" class="form-control" id="InputLast" placeholder="Enter Last Name">
                    <div class="text-danger"><?php echo $ex['elname'];?></div>
                    </div>
                    <div class="form-group ">
                        <label for="InputUser">User Name</label>
                        <input type="text"  name="user_name"  value="<?php  echo (isset($user_update_id)) ?  $updata['user_name'] : $uname; ?>" class="form-control" id="InputUser" placeholder="Enter User Name">
                    <div class="text-danger"><?php echo $ex['euname'];?></div>
                    </div>
                    <div class="form-group ">
                        <label for="InputPass">Password</label>
                        <input type="password"  name="pass" value="<?php  echo (isset($user_update_id)) ?  $updata['user_password'] : $upass; ?>" class="form-control" id="InputPass" placeholder="Enter Password">
